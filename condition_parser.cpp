...
template <class It> shared_ptr<Node> ParseComparison(It& current, It end) {
  ...
  if (column.value == "date") {//
    ...
    return make_shared<DateComparisonNode>(cmp, ParseDate(is));//оператор сравнения дат
  } else {
    return make_shared<EventComparisonNode>(cmp, value);//оператор сравнения событий
  }
}

template <class It>
shared_ptr<Node> ParseExpression(It& current, It end, unsigned precedence) {//precedence-старшинство
  if (current == end) {
  ...
  while (current != end && current->type != TokenType::PAREN_RIGHT) {
    ...
    left = make_shared<LogicalOperationNode>(//узел операции сравнения
...
shared_ptr<Node> ParseCondition(istream& is) {//vector<Token> Tokenize(istream& cl)
  auto tokens = Tokenize(is);
  auto current = tokens.begin();
  auto top_node = ParseExpression(current, tokens.end(), 0u);//pfrse выражение
